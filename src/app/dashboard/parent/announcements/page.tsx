...
export default function ParentAnnouncementsPage() {
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title="Announcements" subtitle="School notices and updates" />
      <div className="space-y-4">
        {loading ? [...Array(3)].map((_, i) => <div key={i} className="h-28 shimmer rounded-2xl" />) :
          announcements.length === 0 ? <Card><div className="text-center py-12 text-slate-400">No announcements.</div></Card> :
          announcements.map((ann: any) => (
            <div key={ann.id} className={`bg-white rounded-2xl border p-5 ${ann.isPinned ? "border-brand-200" : "border-slate-200"}`}>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-400 mt-2"><Clock size={11} /> {formatDate(ann.publishedAt)}</div>
                </div>
              </div>
            </div>
          ))}
